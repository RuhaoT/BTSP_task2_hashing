...
import torch
class btsp_lsh(LSH):
	def __init__(self, data, hash_length, sampling_ratio, embedding_size, binary_mode = False, label=None, lrs = 0.1, device = 'cpu'):
		"""
		data: Nxd matrix
		hash_length: scalar
		...
		plateaus = torch.rand(data.shape[0],embedding_size) < (2*hash_length/embedding_size)
		...
		if not binary_mode:
			sum_spike = 0.
			inp_spike = self.data > torch.rand(self.data.shape[0], self.data.shape[1])
			self.mem = inp_spike.float() @ self.weights
			self.hashes = torch.zeros_like(self.mem)
